...
a stream with NO control plane: when a node dies it (1) re-points the entry node at a PRE-WARMED spare
(serve.py `set_next`), (2) replays the committed history through entry->spare so the spare rebuilds its
KV (the replay's last logit IS the next token — rebuild + resume in one prefill), (3) resumes. Proven
here on the CPU mock (kill a node mid-stream, output must match the no-kill run); sglang-validated live
on a 3+1-box fleet.
...
def decode_with_recovery(head, active, spare, spare_host: str, spare_port: int,
                         prompt: List[int], n_new: int, kill_after=None, kill_fn=None):
    """Greedy-decode `n_new` tokens through head -> … -> active. If `kill_fn` is set, fire it once after
    `kill_after` tokens have committed, then RECOVER onto the pre-warmed `spare`: re-point the entry
    (set_next), read from the spare, replay the committed history (its last logit is the next token —
    KV rebuild + resume in one prefill), continue. Returns (tokens, mttr_s | None)."""
    import torch
    ...
    while len(out) < n_new:
        ...
        out.append(_next_tok(active.recv()))
        pos += cur.shape[1]
        cur = torch.tensor([[out[-1]]])
        if kill_fn is not None and len(out) == kill_after:
            t0 = time.time()
            kill_fn()                                                       # the active (tail) node dies
            ...
            head.send({"h": torch.tensor([hist]), "seq": "s0", "pos": 0})   # last logit = the next token
            out.append(_next_tok(active.recv()))
            pos = len(hist)
            cur = torch.tensor([[out[-1]]])
            mttr = time.time() - t0
            kill_fn = None                                                  # recover once
    return out[:n_new], mttr


def _spawn(runtime, model, ls, le, lp, nh, np_, device="cpu"):
...
def _selftest() -> None:
    ...
    # entry 0..CUT (7911 -> tail 7912); tail CUT..NL (7912 -> driver tail-sink 7914);
    ...
    tail  = _spawn("mock", "mock:8", CUT, NL, 7912, "127.0.0.1", 7914)
    ...
    try:
        ...
        rec, mttr = decode_with_recovery(head, tail_e, spare_e, "127.0.0.1", 7913,
                                         PROMPT, NNEW, kill_after=3, kill_fn=tail.kill)
        print("rec (kill@3)  :", rec, "MATCH", rec == ref, "| MTTR %.3fs" % (mttr or 0))
        try:
            head.send({"op": "stop"})
        except Exception:
            pass
        assert rec == ref, "recovered output != no-kill reference — recovery bug"
        print(">>> RECOVERY OK (mock kill -> re-stitch to warm spare -> replay -> resume == no-kill)")
    finally:
