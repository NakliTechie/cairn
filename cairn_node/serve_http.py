...
# A self-contained chat UI served at GET / (same-origin → no CORS). SSH-tunnel :8000 and open
# http://localhost:8000/ . Model + demo key are injected server-side (__MODEL__ / __KEY__).
_CHAT_HTML = """<!DOCTYPE html><html><head><meta charset="utf-8"><title>Cairn — DeepSeek-V4-Flash</title>
 ...
 *{box-sizing:border-box} body{margin:0;font-family:ui-sans-serif,system-ui,-apple-system,sans-serif;
 ...
 .meta{font-size:11px;color:#9aa49f;margin:4px 8px 0;text-align:right}
 .rec{color:#2f6b25;font-weight:600}
...
<script>
const MODEL="__MODEL__", KEY="__KEY__";
...
const msgs=[];
function add(role,text,meta){const m=document.createElement("div");m.className="msg "+(role==="user"?"u":"a");
  const x=document.createElement("div");x.className="bub";x.textContent=text;m.appendChild(x);log.appendChild(m);
  if(meta){const d=document.createElement("div");d.className="meta";d.innerHTML=meta;log.appendChild(d);}
  log.scrollTop=log.scrollHeight;return x;}
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/", "/chat"):                            # a built-in chat UI (same-origin → no CORS)
            self._send_html(_CHAT_HTML.replace("__MODEL__", self.engine.model)
                            .replace("__KEY__", next(iter(self.engine.gateway.api_keys), "")))
